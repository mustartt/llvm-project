; Negative test for PseudoProbeVerifier on AArch64: a tail call whose call
; instruction carries no pseudo-probe discriminator. PseudoProbeInserter
; therefore emits no call probe before the tail call, so PseudoProbeVerifier
; should warn.

; RUN: llc -mtriple=arm64-apple-macos -stop-after=pseudo-probe-verifier %s -o - 2>&1 | FileCheck %s
...
; scope is !15 directly (no DILexicalBlockFile / pseudo-probe discriminator)
