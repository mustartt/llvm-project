...
const XCOFFRelocation64 *XCOFFObjectFile::getRelocation64(DataRefImpl P) const {
  assert(is64Bit() && "64-bit interface called for non 64-bit file.");
  return viewAs<XCOFFRelocation64>(P.p);
}

const XCOFFRelocation32 *XCOFFObjectFile::getRelocation32(DataRefImpl P) const {
  assert(!is64Bit() && "32-bit interface called for non 32-bit file.");
  return viewAs<XCOFFRelocation32>(P.p);
}
