...
/* Do not change these values.  They are indexes into expanded_ops[]. */
#define D_OP_56 1 /* lq */
#define D_OP_57 2 /* lfdp, --, lxsd, lxssp */
#define D_OP_58 3 /* ld, ldu, lwa, -- */
#define D_OP_62 4 /* std, stdu, stq, -- */

#define OP_MASK 0x7F
#define OP_EXPANDED 0x80
...
static Expected<uint32_t> getDFormInstructionXOMask(const char *InstrPtr) {
  ...
  } else if (FixupInfo.isExpandedOp()) {
    if (FixupInfo.ExpandedOp->OpCode != OpCode)
      return make_error<JITLinkError>("Internal link error");
    XOMask = (1 << FixupInfo.ExpandedOp->XOBits) - 1;
  }

  return XOMask;
}

template <uint32_t Width> bool isOutOfRange(int64_t Value) {
  return Value < -(1 << (Width + 1)) || Value > ((1 << Width) - 1);
}
...
Error applyFixup(LinkGraph &G, Block &B, const Edge &E,
  ...
  case R_TOC_16: {
    ...
    // TODO: Figure out when the displacement is not set by the compiler or
    // when mistmatch. Currently the compiler already generates the displacement
    // from r2.
    break;
  }
  case R_RBR_26: {
    ...
    LLVM_DEBUG(dbgs() << formatv("  R_RBR_26 target address = {:x}", Value));

    if (isOutOfRange<26>(Value))
    ...
    uint32_t BranchInstr = RawInstr | Value;
